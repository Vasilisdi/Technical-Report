...
\subsection{The Raspberry Pi 4 Algorithm} 
{
	...
	The serial communication module, as previously highlighted in the system architecture, establishes a connection with the Arduino microcontroller at a specified baud rate, initializing the serial port with a timeout parameter to prevent blocking operations. Upon initialization, the Raspberry Pi transmits a trigger command ('rasp') to request data from the ADXL335 analog accelerometer, then enters a polling loop that waits until the Arduino responds with sensor readings. The received data string, containing three comma-separated acceleration values from the sensor's X, Y, and Z axes, is decoded from UTF-8 format, stripped of extraneous characters, and parsed into floating-point numerical values representing the triaxial acceleration measurements in g-forces, which are subsequently returned for further processing in the vibration monitoring pipeline, consistent with the data flow described in earlier sections.
	
	Moreover, the API communication for transmitting vibration data to the Supabase backend is implemented through a structured workflow that begins with authentication and configuration validation. Upon class instantiation, the system loads essential credentials including the API key and Supabase URLs from environment variables, performing rigorous validation to ensure all required parameters are present before initializing the HTTP headers with proper content type and authentication tokens. The communication process employs timezone-aware timestamp handling, converting UTC measurement timestamps to Athens time zone and formatting them according to ISO 8601 standards with millisecond precision to maintain temporal consistency across the distributed system.
