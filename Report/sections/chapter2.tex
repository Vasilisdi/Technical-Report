...
There is a tremendous abundance of applications where the vibration analysis is a prerequisite. Readings and records are of vital importance in the context of the predictive maintenance implementation. Most of the industries nowadays strive to make use of accurate preventive maintenance plans, which requires a well-rounded continuous monitoring system, that keeps data of the whole equipment. The ultimate goal of the preventive maintenance plan is the elimination of the unwanted shut-downs due to a machine failure and the increase of plant reliability \cite{DehayemNodem2011}.

Most of the machines need bearings because bearings are mechanical components designed to support rotating shafts and minimize friction. The bearings failure may have a wide variety of causes, lubrication shortage, seal failure, high temperatures, misalignment are some of the causes among others. Therefore vibration analysis focuses on predicting all theses possible causes of bearings failure. Some of the preventive actions against the bearings failure are described bellow \cite{Li2001}.
...
How does vibration diagnosis contribute to operational reliability and cost efficiency in engineering applications? The answer to this question is that the implementation of vibration-analysis-based predictive maintenance helps in the direction of troubleshooting the cause of some issue. In addition, raising awareness on some upcoming failure leads to proper planning, which can prevent greater equipment damage.
}
...
{	
	...
	To begin with, the ultimate objective of the preventive maintenance concept is to improve the dependability. In essence, it is a concept that encompasses different subconcepts like Reliability, Availability, Maintenability and others. The reliability interprets the successful uninterrupted operation of an adequately-performing device or an engineering system. This entails that the operating system must retain not only a low Failure rate, but the most important trait of a device in operation must be the absence of failures so as to claim the title of a "reliable system".
	
	According to Martz, "reliability analysis is the engineering and statistical methodology used to assess the unknown reliability of a device. It may be used qualitatively as a means for identifying the various failure modes and causes, or it may be applied quantitatively, using appropriate failure data, to provide a numerical estimate of the unknown reliability" \cite{Martz2001}. Hence, it is worth it utilizing the aforementioned metrics (MTTF and MTBF), when contemplating reliability, since these constitute the actual metrics of reliability.
	...
	\begin{itemize}
		...
		\item \( R(t) \) represents the probability that the device is still operational at time \( t \) \cite{Martz2001}.
		\item \( F(t) \) is the cumulative distribution function of the time to failure, representing the probability that the system has failed by time \( t \).
	\end{itemize}
	
	\subsubsection{Interpretation of the Mean Time To Failure}
	\indent	
	
	In this regard the MTTF (Mean Time To Failure) can be modeled as the integral of the reliability function \( R(t) \) over time, from zero to infinity:
	...
	The MTBF (Mean Time Between Failures) is another critical metric used in reliability engineering, particularly for repairable systems. It represents the average time between failures for a system or component. For repairable systems, MTBF is calculated as the sum of the operational time divided by the number of failures:
	
	\[
	\text{MTBF} = \frac{\text{Total operational time}}{\text{Number of failures}}
	\]
	
	In this case, the reliability function would be:
	...
	MTBF helps in understanding the reliability of a system over time and is essential for planning maintenance schedules, improving system design, and ensuring continuous operation with minimal downtime. A higher MTBF indicates a more reliable system that experiences fewer failures over its operational life.
	
	
	\subsubsection{Interpretation of the Failure Rate}
	\indent	
	
	The Failure rate, often denoted as \(\lambda\), is defined as the frequency with which an engineered system or component fails. This metric is typically expressed in terms of the number of failures per unit time.
	
	In mathematical term the Failure rate can be expressed as follows: 
	...
	\begin{itemize}
		\item \textbf{Constant Failure Rate:} A constant \( \lambda \) suggests that the system is in its "useful life" period, where the rate of failure is steady and does not change over time.
		\item \textbf{Increasing Failure Rate (IFR):} An increasing \( \lambda(t) \) indicates that the system is aging and becoming more prone to failure.
		\item \textbf{Decreasing Failure Rate (DFR):} A decreasing \( \lambda(t) \) is often observed during the initial "infant mortality" period, where early failures occur, but the remaining items are more reliable over time.
	\end{itemize}
	
	
	\subsubsection{Constant Failure Rate}
	For systems with an exponential distribution of failure times (i.e., a constant failure rate), the failure rate \( \lambda \) is constant over time:
	...
	Availability (A) is typically expressed as a percentage and is calculated using the following formula\cite{Ruane2022}:
	\[
	...
	\begin{itemize}
		...
		\item MTTF (Mean Time To Repair): The average time required to repair a failed system component or mechanism and restore it to operational status.
	\end{itemize}
...
Similarly, Lilly and Marscher have studied the vibration and noise derived from pumps and motors separately \cite{Lilly2008}. In addition, the concept of translational and torsional vibration is introduced. In addition to this, the concept of vibration analysis on piping is studied as well. In this context, the translational vibration analysis is made for the engine-pump-support structure, investigating the lateral vibration.  For this study the usage of high-quality sensors is a prerequisite. There is also the concept of the torsional vibration analysis, which is also subtle and requires special study on the oscillating torque.

In addition to the aforementioned study, Lilly and Marscher investigate also the possibility of using some actuators that may compensate the noise of the motor-pump compound, making the most of the electronically generated sound waves. This technique aims at the cancellation of noise pollution \cite{Lilly2008}.
...
Vibration analysis serves as a versatile tool across industries, from traditional rotating machinery to cutting-edge UAVs. Its evolution—driven by innovations in signal processing, AI, and IoT—continues to expand its applicability and precision in fault detection and predictive maintenance.
}
...
As for the sensor's theory of operation \cite{ADXL335}:
\begin{quote}
ADXL335 is a polysilicon surface-micromachined structure built on top of a silicon wafer. Polysilicon springs suspend the structure over the surface of the wafer and provide a resistance against acceleration forces Deflection of the structure is meas
